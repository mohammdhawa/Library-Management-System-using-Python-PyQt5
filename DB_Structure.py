...
class Research(Model):
    research_title = CharField(unique=True)
    research_description = TextField(null=True)
    code = CharField(unique=True)
    research_subject = CharField()
    research_author = CharField()
    research_language = CharField()
    price = DecimalField()
    date = DateTimeField(default=datetime.datetime.now)
...
db.connect()
db.create_tables([Category, Publisher, Author, Book, Student, Employee, Daily_Movement, History, Employee_Permissions, Research, Sales])